...
import WhatsAppBroadcast from '@/components/ui/WhatsAppBroadcast.vue'
...
function selectShown() {
  const s = new Set(selectedIds.value)
  inv.products.forEach(p => s.add(p.id))
  selectedIds.value = s
}
function clearSelection() { selectedIds.value = new Set() }

const selectedProducts = computed(() =>
  inv.products.filter(p => selectedIds.value.has(p.id))
)
  ...
  <div class="space-y-4">
    ...
    <div class="filter-bar">
      ...
      <div class="bar-actions">
        ...
        <!-- WhatsApp broadcast (multi-item) -->
        <button v-if="selectedIds.size" class="clear-btn" @click="clearSelection">Clear ({{ selectedIds.size }})</button>
        <button class="wa-export-btn" :disabled="selectedIds.size === 0" @click="showBroadcast = true">
          ...
          <span>Broadcast{{ selectedIds.size ? ` (${selectedIds.size})` : '' }}</span>
        </button>
    ...
    >
      <tr v-for="p in inv.products" :key="p.id" class="cursor-pointer hover:bg-gray-50">
        <td class="px-3 py-2.5">
          <input
            type="checkbox"
            class="rounded border-gray-300 text-[#e11d48]"
